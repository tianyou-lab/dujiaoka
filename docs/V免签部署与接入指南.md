...
1. **启用嵌入式 V 免签** 打开
2. **通讯密钥** 点右侧「随机生成」按钮，会自动填入一个 32 位随机串（稍后要和安卓 App 对上）
3. **微信收款码** 粘贴自己微信个人收款码的二维码解析结果，例如 `wxp://f2f0xxxx...`。**不会解析请用** [sojson 在线二维码解析](https://www.sojson.com/qr/deqr.html)
4. **支付宝收款码** 同理，粘贴解析出的 `https://qr.alipay.com/xxxx`
5. **订单超时** 建议 `10` 分钟，**心跳超时** 建议 `60` 秒，**金额错位方向** 选「递增」

> ✅ 与 V 免签原版完全一致：**全站只需要 1 个微信码 + 1 个支付宝码**，订单靠金额错位区分。
> 不要按金额建多个二维码，那是老旧用法。
...
* **支付配置 → V 免签 精确金额收款码（高级）**

> 大多数站长**不需要用**这里，4.1 步填两个全局收款码就够了。

仅以下场景有意义：

* 想给**某个特定金额**投放一张**独立的收款二维码**（比如给 18.88 专用店铺码）
* 多店铺分账、内部对账排查
...
* 类型：1=微信，2=支付宝
* 金额：精确到分，如 `1.99`
* 收款 URL：二维码解析出来的 `https://...` 或 `wxp://...`
* 启用：打开

当用户下单经过金额错位后的 `really_price` 与这里的金额完全一致时，会**覆盖全局收款码**显示这张专用码。
